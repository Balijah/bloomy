      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s(colors).empty}>
            ...
            <Text style={s(colors).emptyText}>No active weather alerts for your locations.</Text>
          </View>
        }
        renderItem={({ item }) => {
          const iconName = ALERT_TYPE_ICONS[item.alertType] ?? "warning-outline";
          const severityColor = SEVERITY_COLORS[item.severity ?? "minor"] ?? SEVERITY_COLORS.minor;
          return (
            <View
              style={[s(colors).alertCard, item.isRead && s(colors).alertCardRead]}
              testID={`alert-card-${item.id}`}
            >
              <View style={[s(colors).alertIcon, { backgroundColor: severityColor + "22" }]}>
                <Ionicons name={iconName as any} size={22} color={severityColor} />
              </View>
              <View style={{ flex: 1 }}>
                <View style={s(colors).alertTop}>
                  <Text style={[s(colors).alertTitle, item.isRead && s(colors).alertTitleRead]}>
                    {item.title}
                  </Text>
                  {!item.isRead && <View style={[s(colors).unreadDot, { backgroundColor: severityColor }]} />}
                </View>
                <Text style={s(colors).alertMsg} numberOfLines={2}>
                  {item.message}
                </Text>
                <Text style={s(colors).alertTime}>
                  {format(parseISO(item.createdAt as unknown as string), "MMM d · h:mm a")}
                </Text>
              </View>
              <View style={s(colors).alertActions}>
                {!item.isRead && (
                  <Pressable
                    style={({ pressed }) => [s(colors).actionBtn, pressed && { opacity: 0.7 }]}
...
const s = (colors: ReturnType<typeof useColors>) =>
  StyleSheet.create({
    ...
    alertTitleRead: { fontFamily: "Outfit_400Regular", color: colors.mutedForeground },
